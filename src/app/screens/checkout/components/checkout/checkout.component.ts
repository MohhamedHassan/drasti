...
export class CheckoutComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    this.settingsService.getSettings().subscribe((res: any) => {
      this.demaEnabled = res?.data?.deema_enabled;
    });
  ...
  getCart() {
    ...
    // }
    this.subscribtion = this.cartService.cartItems.subscribe((res: any) => {
      if (res) {
        this.cartItems = res;
        if (!this.cartItems?.length) {
          ...
          this.cartItems.forEach((item) => {
            if (item?.has_material)
              price +=
                Number(item?.material?.discount) ||
                Number(item?.material?.price);
            if (item?.has_offer)
              price +=
                Number(item?.offer?.discount) || Number(item?.offer?.price);
          });
          this.total = price;
          // if(this.total) {
  ...
  checkout() {
    if (!this.checkoutLoading) {
      if (!!localStorage.getItem('drastitoken')) {
        ...
        this.ids = [];
        this.subscribtion2 = this.cartService
          .getcartProducts()
          .subscribe((res: any) => {
