...
export class RegisterComponent implements OnInit {
  ...
  verificationControl: FormControl = new FormControl('', [Validators.required]);
  emitLogin() {
        ...
        (res: any) => {
          ...
          if (this.cartitems?.length) {
            ...
            this.cartitems.forEach((element) => {
              ...
              if (element?.has_material)
                material_ids.push(element?.material?.id);
            });
            this.cartService
              .addToCart({
                offer_ids: offer_ids,
  ...
  verifyCode() {
    if (
      this.verificationControl.valid &&
      this.registerForm.get('phone')?.valid &&
      !this.verifyOtbLoading
    ) {
